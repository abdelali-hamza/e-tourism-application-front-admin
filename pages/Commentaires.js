...
export async function getServerSideProps() {
  const res = await fetch("http://localhost:3001/lieu/comments")
...
const Comment = ({ initialLieux }) => {
  const [comments, setComments] = useState(initialLieux)
  ...
  const deleteComment = async (idcomm) => {
    const response = await fetch(`http://localhost:3001/admin/delcomm/${idcomm}`, {
      method: "POST",
    });

    if (response.ok) {
      console.log("Lieu created successfully");
      const newArray = comments.filter(item => item.id !== idcomm);
  ...
  return (
    <div className="">
      <div className='text-3xl my-5 font-bold flex flex-row justify-between'>
        Commentaires sur lieux
        <div>
      ...
      {filteredComments.slice(0, 12).map((comment) => (
        <article className="p-6 mb-6 text-base bg-white rounded-lg dark:bg-gray-900">
          <footer className="flex justify-between items-center mb-2">
            ...
            <div className="relative inline-block cursor-pointer" onClick={() => deleteComment(comment.idCommentaire)}>
              <svg
          ...
          <div className="flex items-center mt-4 space-x-4" key={comment.id}>
            <button
              type="button"
