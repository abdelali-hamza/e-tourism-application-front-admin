import Head from "next/head";
import wilayas from '../wilaya.json';
import { useState } from "react";

export async function getServerSideProps() {
    const res = await fetch("http://localhost:3001/lieu/All")
    const lieux = await res.json()
    return {
        props: {
            initialLieux: lieux
        }
    }
}

export default function Home({ initialLieux }) {
    const [places, setPlaces] = useState(initialLieux);
    
    const deleteItemHandler = (goalId) => {
        setPlaces((prevGoals) => {
            const updatedGoals = prevGoals.filter((goal) => goal.id !== goalId);
            return updatedGoals;
        });
    };

    function findWilayaById(id) {
        const foundItem = wilayas.find(item => item.id === id.toString());
        return foundItem ? foundItem.wilaya : null;
    }

    const flippedPlaces = [...places].reverse();


    return (
        <>
            <div className=" items-center justify-center">
                <div className='text-3xl my-5 font-bold flex flex-col'>
                    Lieux touristiques
                </div>
                <>
                    <div className="relative overflow-x-auto shadow-md sm:rounded-lg text">
                        <table className="w-full text-sm text-left text-gray-500 ">
                            <thead className="text-xs text-gray-700 uppercase bg-gray-50 ">
                                <tr>
                                    <th scope="col" className="w-1/4 px-6 py-3">
                                        Lieu
                                    </th>
                                    <th scope="col" className="w-1/4 px-6 py-3">
                                        descreption
                                    </th>
                                    <th scope="col" className="w-1/4 px-6 py-3">
                                        tarif
                                    </th>
                                    <th scope="col" className="w-1/4 px-6 py-3">
                                        Heure Ouverture
                                    </th>
                                    <th scope="col" className="w-1/4 px-6 py-3">
                                        Heure Fermeture
                                    </th>
                                    <th scope="col" className="w-1/4 px-6 py-3">
                                        Wilaya
                                    </th>
                                    <th scope="col" className="w-1/8 px-6 py-3">
                                        Modifier
                                    </th>
                                    <th scope="col" className="w-1/8 px-6 py-3">
                                        Supprimer
                                    </th>
                                </tr>
                            </thead>
                            <tbody>
                                {flippedPlaces.map((place) => (
                                    <tr className="bg-white border-b hover:bg-gray-50" key={place.idLieux}>
                                        <th
                                            scope="row"
                                            className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap "
                                        >
                                            {place.Nom}
                                        </th>
                                        <td className="px-6 py-4">{place.Description}</td>
                                        <td className="px-6 py-4">{place.tarif}</td>
                                        <td className=" px-6 py-4">{place.tempsOuverture}</td>
                                        <td className=" px-6 py-4">{place.tempsFermeture}</td>
                                        <td className=" px-6 py-4">{findWilayaById(place.idwilaya)}</td>
                                        <td className="px-6 py-4 ">
                                            <svg
                                                xmlns="http://www.w3.org/2000/svg"
                                                viewBox="0 0 24 24"
                                                fill="none"
                                                strokeWidth={1.5}
                                                stroke="currentColor"
                                                className="w-6 h-6 hover:stroke-cyan-500"
                                            >
                                                <path
                                                    strokeLinecap="round"
                                                    strokeLinejoin="round"
                                                    d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10"
                                                />
                                            </svg>
                                        </td>
                                        <td className="px-6 py-4 text-right">
                                            <svg
                                                onClick={deleteItemHandler}
                                                xmlns="http://www.w3.org/2000/svg"
                                                fill="none"
                                                viewBox="0 0 24 24"
                                                strokeWidth={1.5}
                                                stroke="currentColor"
                                                className="w-6 h-6 hover:stroke-red-500"
                                            >
                                                <path
                                                    strokeLinecap="round"
                                                    strokeLinejoin="round"
                                                    d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0"
                                                />
                                            </svg>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                    </div>
                </>
            </div >
        </>
    );
}